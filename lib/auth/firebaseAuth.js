...
const sendingPushmsg = async (req, res, next) =>{
    ...
    let {title, body, url, target, compCd, userId, msgType, compNm} = req.body;

    console.log(title," : 제목");
    console.log(body," : 내용");
    console.log(url," : 목표위치");
    console.log(target," : 목표기기");
    console.log(compCd," : 업체 코드");
    console.log(userId," : 업체 유저 아이디 코드");
    console.log(compNm," : 업체명")
    console.log(msgType," : 메시지 타입");
    ...
    if(compCd)msg.data.compCd=compCd;
    if(msgType)msg.data.msgType=msgType;
    if(userId)msg.data.userId=userId;
    if(compNm)msg.data.compNm=compNm;
    if(target){
        if(target=="ALL"){
            msg.topic=target;
        }else{
